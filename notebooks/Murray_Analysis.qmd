...
ggplot(boot_conf_plot, aes(term, y=exp(estimate), ymin=exp(conf.low), ymax=exp(conf.high))) +
  ...
  scale_x_discrete(limits=c('isSizeSame', 'isColSame', 'isPulseSame', 'distToMiddleREV', 'distToNearest', 'n_in_quadrant')) +
...
```{r}
as.data.frame(dredge_res) %>% 
  select(-logLik, -df, -weight, -1) %>% 
  mutate(
    AICc = round(AICc, 0),
    delta = round(delta, 1),
  ) %>% 
